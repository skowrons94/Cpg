...
def createPlot( sim, energy, deltaE, position, scale ):
    ...
    line1, = ax1.step( sim.x, sim.spectrum + sim.spectrumBack, linewidth=2, color='tab:red', label="Signal" )
    line2, = ax1.step( sim.x, sim.spectrumBack, linewidth=2, color='tab:blue', label="Background" )
    ax1.set_xlabel( "Energy (keV)", fontsize=20, horizontalalignment='right', x=1.0 )
    ...
    ax1.legend([line1, line2], ['Signal', 'Background'], loc="upper right", fontsize=20)

    ax2.plot( sim.effPlot[:,0], sim.effPlot[:,1] )
    ax2.scatter( posMarker[0], posMarker[1], color='red', s=70 )
    ax2.set_xlabel( "Position (cm)",  fontsize=13, horizontalalignment='right', x=1.0 )
    ax2.set_ylabel( "Efficiency (%)", fontsize=13, horizontalalignment='right', y=1.0 )
    ax2.tick_params( axis = 'both', which = 'major', labelsize = 13 )
    
    ax3.plot( sim.crossPlot[:,0], sim.crossPlot[:,1] )
    ax3.plot( sim.crossPlot[idx2:idx1,0], sim.crossPlot[idx2:idx1,1], color='red', linewidth=4 )
    ax3.set_xlabel( "Energy (keV)",      fontsize=13, horizontalalignment='right', x=1.0 )
    ax3.set_ylabel( "Cross-section (b)", fontsize=13, horizontalalignment='right', y=1.0 )
    ...
    ax4.plot( xP, yP )
    ax4.set_xlabel( "Beam Energy (keV)", fontsize=13, horizontalalignment='right', x=1.0 )
