...
def Plot( energy = 380, deltaE = 15, position = 1.35, current = 400, time = 2,
    ...
    line1, = ax1.step( sim.x[10:Min], sim.spectrum[10:Min], linewidth=2,
                       color='tab:blue', label="Spectrum" )
    line2, = ax1.step( sim.x[Min-1:Max], sim.spectrum[Min-1:Max], linewidth=2,
                       color='tab:red', label="ROI" )
    ax1.step( sim.x[Max-1:-10], sim.spectrum[Max-1:-10], linewidth=2,
              color='tab:blue')
    ...
    ax1.legend([line1, line2], ['Spectrum', 'ROI'], loc="upper left",
               fontsize=25)
